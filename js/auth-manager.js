...
if (typeof window.AuthManager === 'undefined') {
    ...
    constructor() {
        console.log('[AuthManager] Constructor: Initializing AuthManager instance');
        this.authenticated = false;
        this.user = null;
        this.sessionExpirationCheckInterval = null;
        this.sessionCheckIntervalMs = 60000; // Check session every minute
        this.sessionExpirationThresholdMs = 600000; // 10 minutes
        this.isPublicPage = document.querySelector('meta[name="requires-auth"]') === null;
        
        console.log('[AuthManager] Constructor: Page type detected:', this.isPublicPage ? 'Public Page' : 'Auth Required Page');
        
        // Element references - will be set in setupDOMListeners
        this.logoutButtons = [];
        this.loginLink = null;
    ...
    init() {
        console.log('[AuthManager] init: Starting initialization');
        
        // Setup consistent initialization handling for all pages
        const initializePage = () => {
            console.log('[AuthManager] init: Running full initialization');
            this.checkAuthentication(); // Check auth status from localStorage 
            this.setupDOMListeners(); // Find and set up all interactive elements
            
            // Extra handling for auth-required pages
            if (!this.isPublicPage) {
                console.log('[AuthManager] init: Running protected page checks');
                this.checkProtectedPage();
            }
            
            // Explicitly force UI update to match auth state
            this.updateUI();
            console.log('[AuthManager] init: Complete, UI updated to match auth state:', this.authenticated);
        };
        
        // Use DOMContentLoaded to ensure DOM is fully ready before initialization
        if (document.readyState === 'loading') {
            console.log('[AuthManager] init: DOM still loading, waiting for DOMContentLoaded event');
            document.addEventListener('DOMContentLoaded', initializePage);
        } else {
            console.log('[AuthManager] init: DOM already loaded, running initialization immediately');
            initializePage();
        }
        
        // Also set up a MutationObserver to catch potential SPA navigations or DOM changes
        const observer = new MutationObserver((mutations) => {
            // Only update if we see significant DOM changes that might indicate navigation
            if (mutations.some(m => m.addedNodes.length > 3)) {
                console.log('[AuthManager] Detected significant DOM changes, refreshing UI');
    ...
    checkProtectedPage() {
        ...
        // Use a more precise regex pattern to detect login page and avoid redirect loops
    ...
    checkAuthentication() {
        ...
        if (isAuthenticatedStored) {
            ...
            // Set a global document attribute to assist with UI sync across pages
            ...
            // Clear the global document attribute
            document.documentElement.dataset.userAuthenticated = 'false';
            
            // Stop any active session checks
            this.stopSessionExpirationCheck();
        }
        
        // Always update UI after authentication check
        this.updateUI();
    }
    ...
    setupDOMListeners() {
        ...
        // Register click handlers on logout buttons
        ...
        // Call updateUI to refresh the UI state
        this.updateUI();
    }
    ...
    updateUI() {
        if (!document.body) return;
        console.log('[AuthManager] updateUI: Starting UI update, authenticated =', this.authenticated);
        ...
        console.log('[AuthManager] updateUI: Found DOM elements:', { 
            ...
            mobileLoginLink: !!this.mobileLoginLink 
        });
        
        if (this.authenticated && this.user) {
            console.log('[AuthManager] updateUI: User is authenticated, updating UI');
            document.body.classList.add('user-authenticated');
            ...
            if (this.loginLink && this.userMenuContainer) {
                ...
                // Ensure the member type badge is shown if available
                const memberTypeBadge = document.getElementById('member-type-badge');
                if (memberTypeBadge && this.user.memberType) {
                    memberTypeBadge.textContent = this.user.memberType;
                    memberTypeBadge.classList.remove('hidden');
                }
            } else {
                console.log('[AuthManager] updateUI: Could not find loginLink or userMenuContainer');
            }
            ...
            console.log('[AuthManager] updateUI: User not authenticated, resetting UI');
            document.body.classList.remove('user-authenticated');
            ...
            if (this.loginLink && this.userMenuContainer) {
                ...
                // Hide member type badge if available
                const memberTypeBadge = document.getElementById('member-type-badge');
                if (memberTypeBadge) {
                    memberTypeBadge.classList.add('hidden');
                }
            }
    ...
    login(userData, rememberMe = false) {
        ...
        setTimeout(() => {
            ...
            if (window.location.pathname.includes('/login/')) {
                ...
                // Don't redirect to login page or portal_dashboard (use dashboard instead)
                if (redirectPath && redirectPath !== '/login/' && redirectPath !== '/portal_dashboard/') {
